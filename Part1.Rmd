...
datt=data.frame(logNODG=Log_NODG,Batch=clean_anno[,4],PCA_log=PCA_log$x[,1:2])  # to ease plotting combine results in a dataframe 
```
...
tSNE=Rtsne(X=PCA_log$x[,1:50],dim=2,perplexity=min(50,round(sqrt(nrow(PCA_log$x)))),theta=0.25,pca=F,is_distance = F)  
