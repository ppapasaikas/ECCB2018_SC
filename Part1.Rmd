...
* Fraction of ERCC-originating reads: As mentioned above high fractions of ERCC-originating reads are also indicative of low cell quality. We already say that 
...
* Fraction of MT reads: As noted previously high fractions of MT reads might indicate damaged cells. We will look, as in the case of number of detected genes, for a point in the plot of the fraction of MT reads of the cells sorted by their rank a point where values appear to rise sharply:
```{r filter_high_MT}
...
A second more elaborate filter will aim to remove genes that do not present cell-to-cell fluctuations above of what is expected due to technical variation. 
One possible approach would be to use the mena-variance trend constructed above and keep only the overdispersed genes (genes above the fitted line):
```{r mean_cv_plot2}
...
n_genes_keep=ceiling(0.5*nrow(clean_norm_umi_counts) ) #In the end retain only the top 50% overdispersed genes
norm_GeneDispersion <- select_variable_genes(clean_norm_umi_counts)
disp_cut_off <- sort(norm_GeneDispersion,decreasing=TRUE)[n_genes_keep]
genes_keep <- norm_GeneDispersion >= disp_cut_off
```
...
In order to check and compare later on the effects of selecting only overdispersed genes we will create new gene-filtered version of our data:
```{r filter_genes}
clean_umi_counts2=clean_umi_counts[genes_keep,]
clean_norm_umi_counts2=clean_norm_umi_counts[genes_keep,] #
```
