...
Variation in gene abundance estimates between different cells can be thought of as the convolution of the technical (mainly sampling) and the biological (e.g cell type) sources of variance. Typically one wants to isolate and focus on the biological variance so that the differences due to experimental noise have as small impact as possible on subsequent analyses.  
As might be intuitevely obvious the effects of sampling noise on our estimates of relative gene abundance  decrease with higher levels of gene expression. For example we expect two measurements of a highly expressed gene in two cells of the same type to be more consistent than two measurements of rare genes (where big fold change difference can be expected because of chance). This simple intuition is nicely cpatured in a plot of the gene coefficient of variation (variance normalized by the mean) as a function of the mean gene expression, also known as the mean variance trend:
```{r mean_cv_plot}
...
abline(coef(m)[2],coef(m)[1],col="red",lwd=2,lty=2)
abline(2,-0.5,col="grey",lwd=2,lty=2)
```

## Cell and Gene Filtering
The goal of this section is to perform a clean-up of the dataset in two dimensions:
First to remove low quality cells that might otherwise be mistaken for disting meaningful groupings.
...
To this end we shall used several of the indices introduced in the previous section. 

Although the exact parameters of filtering largely depend on the specific platform used and the specific of the experiment the criteria used below are quire general. As a general guideline we should make an effort to remove  clear outlier cells/noisy genes but still be conservative enough so as to avoid throwing out cells/genes with differences of truly biological origin.

### Cell filtering
We will remove cells according to three criteria introduced above:

* Library size
* Fraction of ERCC-originating reads
* Fraction of MT reads

### Gene filtering
The goal here is to throw out genes that offer no information as to the biological variance of the cells.
A first simple filter is to remove all genes that are not detected in ANY of the cells:

Second we will use the relationship between average gene expression and gene variance to discard genes whose variance is less than what would be expected solely because of sampling noise. A simple way to achieve this is to fit a curve in the  mean to variance trend intorduced above: 


This fit assumes that a simple function captures the relationship between mean gene expression and variance. A better (non-parametric) approach is to assume that this relationship varies in different ranges of gene expression. The function below... 
