...
boxwex=0.5,names=ENS2HGNC[rownames(ReadFraction)[n:1],2],col="orange",
...
m=lm(Y1[endog] ~ X1[endog]) #linear fit of log(cv) as a function of log(gene expression)
...
Notice the high variance of the ERCCs. How does the plot change if we first remove the problematic batch (NA19098.r2) identified above?

```{r mean_cv_plot_rem}
use.data=norm_umi_counts[,-c(grep("r2",colnames(umi_counts)) )] # Remove the problematic batch (NA19098.r2)
DetectedGenes=which(rowSums(use.data)>0) #Genes that are detected in at least one cell
mean_GE=rowMeans(use.data[DetectedGenes,  ] +1/ncol(use.data)) #Mean expresion (with pseudocounts)
gene_cv=apply(use.data[DetectedGenes,  ],1, function(x) sd(x)/mean(x+1/length(x)) ) #coefficient of variation
X1=log2(mean_GE) #Log transform expression
...
#mt vector already contains the ids of the mitochondrial genes.
...
We suggest removing these genes during the first steps of the analysis (as they can be a source of large spurious variance between cells):
...
cells_remove <- unique(c(filtered_cells,grep("NA19098.r2",colnames(umi_counts))))

# Select Genes (Remove low expressed genes)
filter_genes <- apply(umi_counts[,-cells_remove], 1, function(x) length(x[x > 1]) >= 2)

umi.qc <- as.matrix(umi_counts[filter_genes,-cells_remove])
anno.qc <- anno[-cells_remove,]
erccs.qc <- rownames(umi.qc)[(grepl("ERCC-", rownames(umi.qc)))]
endog.qc <- rownames(umi.qc)[(grepl("ENSG", rownames(umi.qc)))]
umi.qc.norm <- t(t(umi.qc[endog.qc,]) /colSums(umi.qc[endog.qc,]) *1e6)
...
Log_NODG=log2(colSums(umi.qc.norm>0)+1) # Calculate also the number of detected genes (to use later in plotting)

genes_keep <- endog.qc[select_variable_genes(umi.qc.norm[endog.qc,],0.25)] #Select the top 25% of overdispersed genes
LogTransf_counts=log2(umi.qc.norm[genes_keep,]+1)
PCA_Sel=prcomp(x=t(LogTransf_counts),scale=T)

genes_keep <- endog.qc #All endogenous genes
LogTransf_counts=log2(umi.qc.norm[genes_keep,]+1)
...
```{r confounding_pca1}
datt=data.frame(logNODG=Log_NODG,Batch=anno.qc[,4],Cell_Cycle=ccpat,PCA=PCA$x[,1:2])  # to ease plotting combine results in a 
...
```
```{r confounding_pca3}
chart_cc=ggplot(datt,aes(x=PCA.PC1,y=PCA.PC2,color=Cell_Cycle))
chart_cc=chart_cc+geom_point(size=2,alpha=0.8)
grid.arrange(chart_cc, ncol=1)
```
...
```
```{r confounding_tsne3}
chart_cc=ggplot(datt,aes(x=tSNE.1,y=tSNE.2,color=Cell_Cycle))
chart_cc=chart_cc+geom_point(size=2,alpha=0.8)
grid.arrange(chart_cc, ncol=1)
```
...
To estimate the factors of unwanted variation, we need a set of negative control genes, i.e., genes that can be assumed not to be influenced by the covariates of interest. In many cases, such a set can be identified, e.g., housekeeping genes or spike-in controls. Here, we use the ERCC spike-ins as controls and we consider k = 1 factors of unwanted variation. 
...
library(RUVSeq)
# RUVg
ruvg <- RUVg(umi.qc, erccs.qc, k = 1)
umi.qc.ruvg1 <- ruvg$normalizedCounts
ruvg <- RUVg(umi.qc, erccs.qc, k = 2)
umi.qc.ruvg2 <- ruvg$normalizedCounts
umi.qc.ruvg2_logcpm <- log2(t(t(ruvg$normalizedCounts) / colSums(ruvg$normalizedCounts) * 1e6) + 1)

```


```{r ruvg1_pca, message=FALSE}
LogTransf_counts=log2(umi.qc.ruvg1[endog.qc,]+1)
PCA=prcomp(x=t(LogTransf_counts),scale=F)
...
grid.arrange(chart_logNODG, chart_batch, chart_cc, ncol=2, nrow=2)
```



```{r ruvg2_pca, message=FALSE}
LogTransf_counts=log2(umi.qc.ruvg2[endog.qc,]+1)
PCA=prcomp(x=t(LogTransf_counts),scale=F)
Log_NODG=log2(colSums(umi.qc>0)+1) # Calculate also the number of detected genes (to use later in plotting)
datt=data.frame(logNODG=Log_NODG,Batch=anno.qc[,4],Cell_Cycle=ccpat,PCA=PCA$x[,1:2])  # to ease plotting combine results in a dataframe
...
#anno.qc <- anno[anno$sample_id %in% colnames(umi.qc),]
# RUVs
scIdx <- matrix(-1, ncol = max(table(anno.qc$individual)), nrow = 3)
tmp <- which(anno.qc$individual == "NA19098")
scIdx[1, 1:length(tmp)] <- tmp
tmp <- which(anno.qc$individual == "NA19101")
scIdx[2, 1:length(tmp)] <- tmp
tmp <- which(anno.qc$individual == "NA19239")
scIdx[3, 1:length(tmp)] <- tmp
cIdx <- rownames(umi.qc)

#ruvs <- RUVs(umi.qc, cIdx, k = 1, scIdx = scIdx, isLog = FALSE)
#saveRDS(ruvs,paste0(PATH,"data/ruvs1.rds"))
ruvs <- readRDS(paste0(PATH,"data/ruvs1.rds"))
umi.qc.ruvs1 <- ruvs$normalizedCounts

#ruvs <- RUVs(umi.qc, cIdx, k = 2, scIdx = scIdx, isLog = FALSE)
#saveRDS(ruvs,paste0(PATH,"data/ruvs2.rds"))
ruvs <- readRDS(paste0(PATH,"data/ruvs2.rds"))
umi.qc.ruvs2 <- ruvs$normalizedCounts

umi.qc.ruvs2_logcpm <- log2(t(t(ruvs$normalizedCounts) / colSums(ruvs$normalizedCounts) * 1e6) + 1)
```


```{r ruvs1_pca, message=FALSE}
LogTransf_counts=log2(umi.qc.ruvs1[endog.qc,]+1)
PCA=prcomp(x=t(LogTransf_counts),scale=F)
Log_NODG=log2(colSums(umi.qc>0)+1) # Calculate also the number of detected genes (to use later in plotting)
datt=data.frame(logNODG=Log_NODG,Batch=anno.qc[,4],Cell_Cycle=ccpat,PCA=PCA$x[,1:2])  # to ease plotting combine results in a dataframe
...
LogTransf_counts=log2(umi.qc.ruvs2[endog.qc,]+1)
PCA=prcomp(x=t(LogTransf_counts),scale=F)
Log_NODG=log2(colSums(umi.qc>0)+1) # Calculate also the number of detected genes (to use later in plotting)
datt=data.frame(logNODG=Log_NODG,Batch=anno.qc[,4],Cell_Cycle=ccpat,PCA=PCA$x[,1:2])  # to ease plotting combine results in a dataframe
...
```{r ruvs2_logcpm_pca, message=FALSE}
LogTransf_counts=log2(umi.qc.ruvs2_logcpm[endog.qc,]+1)
PCA=prcomp(x=t(LogTransf_counts),scale=F)
Log_NODG=log2(colSums(umi.qc>0)+1) # Calculate also the number of detected genes (to use later in plotting)
datt=data.frame(logNODG=Log_NODG,Batch=anno.qc[,4],Cell_Cycle=ccpat,PCA=PCA$x[,1:2])  # to ease plotting combine results in a dataframe
...
As we have seen in the previouse section part of the observed variation is coming from sources that are either not biological (e.g technical batch, library size) or that is biological in nature but trivial and unrelated to the question we want to analyze (cells in different cell cycle stages or apoptotic vs healthy cells are common examples).
...
They are also non-linear meaning that they can identify clusters that are not separable under linear data transformations. Examples of such algorithms are *SIMLR*, *PhenoClust* and the algorithm used in this tutorial *griph*. Typically these approaches perform well (better) than more classical density-based clustering approaches (e.g k-means).
...
Comparison of individual NA19101 to NA19239
...
keep <- clean_anno[,1] == "NA19101" | clean_anno[,1] == "NA19239"
...
We'll fit a hurdle model, modeling the condition and cngeneson factor, thus adjusting for the cellular detection rate. In order to have more interpretable coefficients, we'll set the reference level of the factor to be the "NA19101" cells.
...
cond <- relevel(colData(obj)$cond,"NA19101")
...
fcHurdle <- merge(summaryDt[contrast=='condNA19239' & component=='H',.(primerid, `Pr(>Chisq)`)], # hurdle P values
                  summaryDt[contrast=='condNA19239' & component=='logFC', .(primerid, coef, ci.hi, ci.lo)], by='primerid') #logFC coefficients
...
# Load human GSEA hallmarks geneset Hs.H
load(paste0(PATH,"data/human_H_v5p2.rdata")) # H hallmark gene sets # Hs.H
names(Hs.H) <- gsub("HALLMARK_","",names(Hs.H))
sets_indices <- limma::ids2indices(Hs.H, rownames(clean_norm_umi_counts))
...
```{r MAST_GSEA_2, message=FALSE, eval=FALSE}
