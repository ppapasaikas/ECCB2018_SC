...
qq <- table(gene_mapper) # removes NA in ENSEMBL already
...
genes_keep <- endog.qc[select_variable_genes(umi.qc.norm,0.25)] #Select the top 25% of overdispersed genes
...
plot(summary(PCA_Sel)$importance[3,1:50],type="l",xlab="#PC (LogTrans. Data)",ylab="Cum. Proportion of Explained Variance")
datt=data.frame(logNODG=Log_NODG,Batch=ccpat,PCA_Sel=PCA_Sel$x[,1:2])  # to ease plotting combine results in a 
chart_batch=ggplot(datt,aes(x=PCA_Sel.PC1,y=PCA_Sel.PC2,color=Batch))
chart_batch=chart_batch+geom_point(size=4,alpha=0.8)
grid.arrange(chart_batch, ncol=1)

tSNE=Rtsne(X=PCA_Sel$x[,1:50],dim=2,perplexity=min(50,round(sqrt(nrow(PCA_Sel$x)))),theta=0.25,pca=F,is_distance = F)  
datt=data.frame(logNODG=Log_NODG,Batch=anno.qc[,4],tSNE=tSNE$Y)  # to ease plotting combine results in a dataframe 
chart_tSNE1=ggplot(datt,aes(x=tSNE.1,y=tSNE.2,color=ccpat))
chart_tSNE1=chart_tSNE1#+scale_color_gradient(low="#FF5544",high="#4455FF")
chart_tSNE1=chart_tSNE1+geom_point(size=1,alpha=0.8)
chart_tSNE2=ggplot(datt,aes(x=tSNE.1,y=tSNE.2,color=Batch))
chart_tSNE2=chart_tSNE2+geom_point(size=2,alpha=0.8)
grid.arrange(chart_tSNE1, chart_tSNE2, ncol=1)


#LogTransf_counts=log2(umi.qc[endog.qc,]+1)
PCA_noSel=prcomp(x=t(LogTransf_counts),scale=F) #Note that for prcomp we need to pass a trnasposed version of the matrix where cells are the rows and genes the columns
plot(summary(PCA_noSel)$importance[3,1:50],type="l",xlab="#PC (LogTrans. Data)",ylab="Cum. Proportion of Explained Variance")
Log_NODG=log2(colSums(umi.qc[endog.qc,]>0)+1) # Calculate also the number of detected genes (to use later in plotting)
datt=data.frame(logNODG=Log_NODG,Batch=ccpat,PCA_noSel=PCA_noSel$x[,1:2])  # to ease plotting combine results in a 
chart_batch=ggplot(datt,aes(x=PCA_noSel.PC1,y=PCA_noSel.PC2,color=Batch))
chart_batch=chart_batch+geom_point(size=4,alpha=0.8)
grid.arrange(chart_batch, ncol=1)
...
```{r effectiveness1_pca, message=FALSE}

x <- umi.qc.ruvs2
LogTransf_counts=log2(x[endog.qc,]+1)
PCA_noSel=prcomp(x=t(LogTransf_counts),scale=F) #Note that for prcomp we need to pass a trnasposed version of the matrix where cells are the rows and genes the columns
#plot(summary(PCA_noSel)$importance[3,1:50],type="l",xlab="#PC (LogTrans. Data)",ylab="Cum. Proportion of Explained Variance")
Log_NODG=log2(colSums(umi.qc>0)+1) # Calculate also the number of detected genes (to use later in plotting)
datt=data.frame(logNODG=Log_NODG,Batch=anno.qc[,4],PCA_noSel=PCA_noSel$x[,1:2])  # to ease plotting combine results in a dataframe 
...
saveRDS(zlmCond,"data/zlmCond.rds")
```
...
saveRDS(summaryCond,"data/summaryCond.rds")
...
load("data/human_H_v5p2.rdata") # H hallmark gene sets # Hs.H
...
#saveRDS(boots,"data/boots.rds")

# Just load pre-computed data
boots <- readRDS("data/boots.rds")
