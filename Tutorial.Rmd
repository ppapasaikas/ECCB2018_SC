...
[dataset](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5206706/) of
 induced pluripotent stem cells generated from three different individuals  (A, B and C) with three replicates of 96 cells each (A.r1, A.r2, A.r3, ...).
...
molecular identifiers (UMIs, see figure below) and ERCC _spike-ins_ were used. The data files are located in the `data\tung` folder in the [/home/radmin/BC2_SingleCell] directory .
...
library(RUVSeq)
# RUVg
ruvg1 <- RUVg(umi.qc.norm, erccs.qc, k = 1)
umi.qc.ruvg1 <- ruvg1$normalizedCounts
#umi.qc.ruvg1 <- log2(t(t(ruvg1$normalizedCounts) / colSums(ruvg1$normalizedCounts) * 1e6) + 1)

ruvg2 <- RUVg(umi.qc.norm, erccs.qc, k = 2)
umi.qc.ruvg2 <- ruvg2$normalizedCounts
#umi.qc.ruvg2 <- log2(t(t(ruvg2$normalizedCounts) / colSums(ruvg2$normalizedCounts) * 1e6) + 1)
...
PCA=prcomp(x=t(umi.qc.ruvg1[endog.qc,]),scale=T)
tSNE=Rtsne(X=PCA$x[,1:50],dim=2,perplexity=min(50,round(sqrt(nrow(PCA$x)))),theta=0.25,pca=F,is_distance = F)  
...
PCA=prcomp(x=t(umi.qc.ruvg2[endog.qc,]),scale=T)
tSNE=Rtsne(X=PCA$x[,1:50],dim=2,perplexity=min(50,round(sqrt(nrow(PCA$x)))),theta=0.25,pca=F,is_distance = F)  
...
PCA=prcomp(x=t(umi.qc.ruvs1),scale=T)
...
PCA=prcomp(x=t(umi.qc.ruvs2),scale=T)
datt=data.frame(logNODG=Log_NODG,Batch=anno.qc[,4],Cell_Cycle=ccpat,PCA=PCA$x[,1:2])  # to ease plotting combine results in a dataframe
chart_logNODG=ggplot(datt,aes(x=PCA.PC1,y=PCA.PC2,color=logNODG))
chart_logNODG=chart_logNODG+scale_color_gradient(low="#FF5544",high="#4455FF")
chart_logNODG=chart_logNODG+geom_point(size=2,alpha=0.8)
...
griph_res <- griph_cluster(as.matrix(clean_umi_counts[endog,]), plot=FALSE,rho=0.4, use.par = FALSE)
...
griph_res_batch <- griph_cluster(as.matrix(clean_umi_counts[endog,]), BatchAssignment=clean_anno[,4], batch.penalty=1 ,plot=FALSE,rho=0.4, use.par = FALSE)
...
In this last section we will try to list these topics and provide links to online material for those wishing to dig into them:
