...
PATH=""
umi_counts <- read.table(paste(PATH, "data/tung/molecules.txt",sep="") , sep = "\t")
read_counts <- read.table(paste(PATH, "data/tung/molecules.txt",sep="") , sep = "\t")
anno <- read.table(paste(PATH, "data/tung/molecules.txt",sep="") , sep = "\t", header = TRUE)
```
...
plot(X1[endog],Y1[endog],xlab="log2(mean gene expression)",ylab="log2(coefficent of variation)" ,main="mean-variance trend" )
...
points(X1[ERCC],Y1[ERCC],pch=19,col="green")
```
...
griph_res <- griph_cluster(as.matrix(clean_norm_umi_counts[endog,]), ClassAssignment=clean_anno[,4],plotG=FALSE,rho=0.4)
plotLVis(griph_res, fill.type = "true", line.type = "predicted", mark.type="custom", custom.class =clean_anno[,1])
```
...
griph_res_batch <- griph_cluster(as.matrix(clean_norm_umi_counts[endog,]),BatchAssignment=clean_anno[,4], ClassAssignment=clean_anno[,4],batch.penalty=1 ,plotG=FALSE,rho=0.4)
plotLVis(griph_res_batch, fill.type = "true", line.type = "predicted", mark.type="custom", custom.class =clean_anno[,1] )
```
